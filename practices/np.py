...
# --- Cumulate sum and product.
data = np.arange(1, 13).reshape(2, 6)
print(data)
print(np.cumsum(data))
print(np.cumprod(data, axis=0))
print(np.cumprod(data, axis=1), end=SKIP_ONE_LINE)

# --- Sorting names by sorted scores.
scores = np.random.randint(1, 101, size=6)
names = np.array(["Adele", "BiBi", "Carol", "Drek", "Ember", "Frank"])
sortedScore = np.argsort(scores)
print(
    f"Scores = {scores}\nNames = {names}\nSorted scores = {sortedScore}\nSorted names = {names[sortedScore]}",
    end=SKIP_ONE_LINE,
)
